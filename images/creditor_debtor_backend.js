/*
Creditors & Debtors Backend (Single-file Express + SQLite)
File: creditor-debtor-backend.js

Overview:
- Node.js + Express + SQLite (better-sqlite3)
- JWT authentication with role-based access (creditor, debtor, admin)
- Features implemented:
  - Auth (register/login) - sample seed users created on first run
  - Creditor endpoints: dashboard, member management (CRUD), loan approval, repayment tracking
  - Debtor endpoints: view my loans, loan details, monthly installment, penalty status, payment history
  - Late payment monitoring and simple reporting

Not production-ready: intended as a complete starting backend to extend.

Run instructions:
1. Install Node.js (v14+ recommended)
2. Create a folder and place this file in it
3. Run:
   npm init -y
   npm install express better-sqlite3 jsonwebtoken bcryptjs body-parser uuid
4. Start the app:
   node creditor-debtor-backend.js
5. The server listens on port 3000 by default.

Notes:
- The DB will be created in the same folder as 'app.db'
- The app seeds an admin creditor and two sample debtors and a sample loan
- JWT secret is inside this file for simplicity; in production use environment variables
...
function seedSampleData() {
  ...
  // Create a sample loan for Alice (pending)
...
function audit(action, payload) {
  const stmt = db.prepare('INSERT INTO audit_log (id,action,payload) VALUES (?,?,?)');
  stmt.run(uuidv4(), action, JSON.stringify(payload));
}
...
// ------------------ Creditor Management (Admin/Creditor) ------------------

// List creditors (admin/creditor)
app.get('/creditors', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  const rows = db.prepare('SELECT c.id, u.name, u.email, c.company, c.phone FROM creditors c JOIN users u ON c.user_id = u.id').all();
  res.json(rows);
});

// Update creditor profile (creditor owns their record or admin)
app.put('/creditors/:id', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  const id = req.params.id;
  const { company, phone } = req.body;
  const creditor = db.prepare('SELECT * FROM creditors WHERE id = ?').get(id);
  if (!creditor) return res.status(404).json({ error: 'Not found' });

  // if role is creditor, ensure they own it
  if (req.user.role === 'creditor') {
    const own = db.prepare('SELECT user_id FROM creditors WHERE id = ?').get(id);
    if (!own || own.user_id !== req.user.id) return res.status(403).json({ error: 'Forbidden' });
  }

  db.prepare('UPDATE creditors SET company = ?, phone = ? WHERE id = ?').run(company || creditor.company, phone || creditor.phone, id);
  audit('update_creditor', { id, by: req.user.id });
  res.json({ message: 'updated' });
});

// ------------------ Debtor Management (Admin/Creditor) ------------------
// Create debtor
app.post('/debtors', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  const { name, email, password, national_id, phone, address } = req.body;
  ...
  const uid = uuidv4();
  const pwd = bcrypt.hashSync(password, 8);
  db.prepare('INSERT INTO users (id,name,email,password,role) VALUES (?,?,?,?,?)').run(uid, name, email, pwd, 'debtor');
  const did = uuidv4();
  db.prepare('INSERT INTO debtors (id,user_id,national_id,phone,address) VALUES (?,?,?,?,?)').run(did, uid, national_id || '', phone || '', address || '');
  audit('create_debtor', { uid, did, by: req.user.id });
  res.json({ message: 'debtor created', debtor_id: did });
});

// List debtors
app.get('/debtors', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  const rows = db.prepare('SELECT d.id, u.name, u.email, d.national_id, d.phone, d.address FROM debtors d JOIN users u ON d.user_id = u.id').all();
  res.json(rows);
});

// Get debtor details
app.get('/debtors/:id', authMiddleware, roleMiddleware(['admin','creditor','debtor']), (req, res) => {
  const id = req.params.id;
  // If debtor role, ensure they can only access their own
  if (req.user.role === 'debtor') {
    const record = db.prepare('SELECT d.id FROM debtors d JOIN users u ON d.user_id = u.id WHERE u.id = ?').get(req.user.id);
    if (!record || record.id !== id) return res.status(403).json({ error: 'Forbidden' });
  }
  const row = db.prepare('SELECT d.id, u.name, u.email, d.national_id, d.phone, d.address FROM debtors d JOIN users u ON d.user_id = u.id WHERE d.id = ?').get(id);
  if (!row) return res.status(404).json({ error: 'Not found' });
  res.json(row);
});

// ------------------ Loans ------------------
// Create loan (creditor creates loan for a debtor, status pending)
...
// Approve/reject loan (creditor or admin) - creditor must own the loan
app.post('/loans/:id/decision', authMiddleware, roleMiddleware(['creditor','admin']), (req, res) => {
  ...
  // if creditor, ensure they own it
  if (req.user.role === 'creditor') {
    const creditor = db.prepare('SELECT id FROM creditors WHERE user_id = ?').get(req.user.id);
    if (!creditor || creditor.id !== loan.creditor_id) return res.status(403).json({ error: 'Forbidden' });
  }

  if (action === 'reject') {
    db.prepare('UPDATE loans SET status = ? WHERE id = ?').run('rejected', id);
    audit('loan_rejected', { id, by: req.user.id });
    return res.json({ message: 'loan rejected' });
  }

  // Approve: compute monthly installment using simple amortization
...
// List loans (creditor sees their loans, admin sees all, debtor sees own loans)
app.get('/loans', authMiddleware, (req, res) => {
  if (req.user.role === 'creditor') {
    const creditor = db.prepare('SELECT id FROM creditors WHERE user_id = ?').get(req.user.id);
    const rows = db.prepare('SELECT l.*, u.name as debtor_name FROM loans l JOIN debtors d ON l.debtor_id = d.id JOIN users u ON d.user_id = u.id WHERE l.creditor_id = ?').all(creditor.id);
    return res.json(rows);
  }
  if (req.user.role === 'debtor') {
    const debtor = db.prepare('SELECT id FROM debtors WHERE user_id = ?').get(req.user.id);
    const rows = db.prepare('SELECT l.*, u.name as creditor_name FROM loans l JOIN creditors c ON l.creditor_id = c.id JOIN users u ON c.user_id = u.id WHERE l.debtor_id = ?').all(debtor.id);
    return res.json(rows);
  }
  // admin
  const rows = db.prepare('SELECT l.*, d.id as debtor_id, du.name as debtor_name, c.id as creditor_id, cu.name as creditor_name FROM loans l JOIN debtors d ON l.debtor_id = d.id JOIN users du ON d.user_id = du.id JOIN creditors c ON l.creditor_id = c.id JOIN users cu ON c.user_id = cu.id').all();
  res.json(rows);
});

// Get loan by id (with repayment summary)
app.get('/loans/:id', authMiddleware, (req, res) => {
  ...
  if (req.user.role === 'debtor') {
    const debtor = db.prepare('SELECT id FROM debtors WHERE user_id = ?').get(req.user.id);
    if (!debtor || debtor.id !== loan.debtor_id) return res.status(403).json({ error: 'Forbidden' });
  }
  if (req.user.role === 'creditor') {
    const creditor = db.prepare('SELECT id FROM creditors WHERE user_id = ?').get(req.user.id);
    if (!creditor || creditor.id !== loan.creditor_id) return res.status(403).json({ error: 'Forbidden' });
  ...
  const outstanding = (loan.monthly_installment ? loan.monthly_installment * loan.term_months : loan.principal) - paid;

  res.json({ loan, repayments, paid, outstanding });
});

// ------------------ Repayments ------------------
// Make a repayment (debtor)
app.post('/loans/:id/repay', authMiddleware, roleMiddleware(['debtor']), (req, res) => {
  ...
  const rid = uuidv4();
  db.prepare('INSERT INTO repayments (id,loan_id,amount,method,note) VALUES (?,?,?,?,?)').run(rid, id, amount, method || '', note || '');
  audit('repayment', { id: rid, loan_id: id, amount, by: req.user.id });

  // Optionally, close loan if fully paid (very simple logic)
  const repayments = db.prepare('SELECT SUM(amount) as s FROM repayments WHERE loan_id = ?').get(id);
  const paid = repayments.s || 0;
  const totalDue = (loan.monthly_installment ? loan.monthly_installment * loan.term_months : loan.principal);
  if (paid + 0.0001 >= totalDue) {
    db.prepare('UPDATE loans SET status = ? WHERE id = ?').run('closed', id);
    audit('loan_closed', { loan_id: id });
  }
...
app.get('/dashboard', authMiddleware, (req, res) => {
  // return different views depending on role
  if (req.user.role === 'creditor') {
    const creditor = db.prepare('SELECT id FROM creditors WHERE user_id = ?').get(req.user.id);
...
// Late payment monitoring: simple heuristic — loan is active and repayments made < number of months elapsed * monthly_installment
app.get('/reports/late', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  ...
  const late = [];
  loans.forEach(l => {
    if (creditorId && l.creditor_id !== creditorId) return; // skip others
    if (!l.start_date) return; // can't compute
    ...
    if (deficit > 0.01) {
      late.push({ loan_id: l.id, debtor_id: l.debtor_id, creditor_name: l.creditor_name, monthsElapsed, expectedPaid, paid, deficit });
    }
  });
...
// Simple reporting: payments between dates
app.get('/reports/payments', authMiddleware, roleMiddleware(['admin','creditor']), (req, res) => {
  const { from, to } = req.query; // ISO dates
  let stmt;
  if (from && to) stmt = db.prepare('SELECT p.*, l.debtor_id FROM repayments p JOIN loans l ON p.loan_id = l.id WHERE date(p.paid_on) BETWEEN date(?) AND date(?) ORDER BY p.paid_on DESC');
  else stmt = db.prepare('SELECT p.*, l.debtor_id FROM repayments p JOIN loans l ON p.loan_id = l.id ORDER BY p.paid_on DESC LIMIT 200');
  const rows = (from && to) ? stmt.all(from, to) : stmt.all();
  res.json({ rows });
});

// ------------------ Debtor endpoints (my loans, payment history) ------------------
app.get('/me/loans', authMiddleware, roleMiddleware(['debtor']), (req, res) => {
  const debtor = db.prepare('SELECT id FROM debtors WHERE user_id = ?').get(req.user.id);
  const rows = db.prepare('SELECT l.*, cu.name as creditor_name FROM loans l JOIN creditors c ON l.creditor_id = c.id JOIN users cu ON c.user_id = cu.id WHERE l.debtor_id = ?').all(debtor.id);
  res.json(rows);
});

app.get('/me/loans/:id/payments', authMiddleware, roleMiddleware(['debtor']), (req, res) => {
  const loanId = req.params.id;
  const debtor = db.prepare('SELECT id FROM debtors WHERE user_id = ?').get(req.user.id);
  const loan = db.prepare('SELECT * FROM loans WHERE id = ?').get(loanId);
  if (!loan) return res.status(404).json({ error: 'Loan not found' });
  if (loan.debtor_id !== debtor.id) return res.status(403).json({ error: 'Forbidden' });
  const rows = db.prepare('SELECT * FROM repayments WHERE loan_id = ? ORDER BY paid_on DESC').all(loanId);
  res.json(rows);
});

// ------------------ Simple utilities ------------------
app.get('/', (req, res) => res.send('Creditors-Debtors Backend Running'));
...
/*
Possible next steps / improvements (not implemented here):
- Pagination and filtering for list endpoints
- Notifications for late payments
- More robust amortization / interest accrual logic
- Attachments, KYC documents
- Role separation (separate admin vs creditor account types)
- Input validation with a library like Joi
- Move JWT secret to environment variables
- HTTPS, Rate limiting, CORS configuration
*/
